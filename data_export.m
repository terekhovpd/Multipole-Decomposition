...
% ver 5.1
% 19.03.2017
% Allows to export COMSOL data to .dat files for convinient usage
...
dimension = in_to_matlab(); % Массив ячеек. Первая char, вторая значение на которое надо умножить
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if size(FRE,1) == 1
    ...
    i = 1;
    while i < size(FRE,1) & FRE(i) == FRE(i+1)
        i = i + 1;
    end
...
length_fre = size(FRE,1);	% number of frequency steps, frequency vector length
...
absCS = absCS ./ 1.3E-3; % absorption normalization by Poynting vector of incident radiation
                         % нормировка поглощения на вектор пойнтинга падающего излучения
