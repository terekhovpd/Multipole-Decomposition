...
MaxTx = ones(4, n_max); 				% создаём массив единиц, 3 строки и n_max стобцов
[MaxTx(1,:), MaxTx(2,:)] = max(Tx);	% в первую строку записываем максимальные значения из каждой колонки
										% во вторую строку записываем номер строки соответсвующий максимальному значению
MaxTx(2,:) = fre(MaxTx(2,:), 1);
MaxTx(3,:) = c ./ MaxTx(2,:);
MaxTx(4,:) = 1:1:n_max;
MaxTx(4,:) = H(1, MaxTx(4,:));
